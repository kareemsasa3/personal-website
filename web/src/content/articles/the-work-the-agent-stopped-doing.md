...
Anastasis holds machine-specific configuration and operational knowledge. Its README puts the ownership boundary plainly:

> Anastasis is the machine; Erebus is software running on the machine.

The general rule is that configuration belongs to the component that owns its meaning. Installing an application's service on the workstation does not make its semantics Anastasis's responsibility.

Themis handles a different problem: governing what a portfolio can justifiably claim about its projects. Its standards are separate from a portfolio instance's authoritative data, and substantive project evidence stays with its owner. Its Work Control model, where adopted, uses Sources, Inputs, Tasks, dispositions, and transitions; a finding does not automatically establish an obligation. Task priority, governance attention, and investment choices answer different questions. This is not Anastasis's commitment model under another name.

Both systems informed my initial observation. Anastasis supplied the episode that made its limits clear.
...
The recording mechanism kept going. Findings received numbered backlog IDs. Resolved items accumulated beside unresolved ones. A cosmetic observation and a demonstrated backup defect went through the same instrument, whether or not anyone had decided to act on them.
...
On September 17, 2026, Anastasis adopted a different maintenance regime. A numbered item now means an explicit commitment: Anastasis owns the matter, observed state diverges from stated intent, and a decision to fix it has been taken. Other findings remain dated observations in the section that owns their subject. They can matter without becoming work.

The distinction had to survive the existing backlog. A one-time pass through twelve open legacy P2 items produced **four promotions to commitments, six demotions to observations, and two transfers to the repositories that owned them**.

Two dispositions explain the change better than the count.

**P2-17 stayed work.** A dump-pruning script deleted local database dumps by age without checking that a replacement had arrived. During a service lapse, it had emptied that service's local dump shelf. Borg archives remained, so this was not total data loss, but it was an observed failure in the backup path. The P2 severity label did not make it optional by itself. The conversion preserved it as an explicit commitment to change the behavior.

**P2-11 became an observation.** A script's non-executable mode and its history were recorded. What had not been established was intent. The record did not justify declaring the script deliberately non-executable, and it did not justify declaring it defective. Demotion preserved what was known without inventing an answer to make the entry feel complete.

One case preserved a decision. The other preserved the absence of one.

The transfers enforced the ownership boundary too. The camera service and access-configuration issue belonged with the application that defined their semantics. Its presence on the workstation did not make Anastasis the place to fix it.

After the conversion, the current commitments were exactly **P1-5, P2-9, P2-10, P2-12, and P2-17**: the existing P1 recovery item and the four surviving P2 commitments. Retired IDs remained explainable through a frozen conversion record. Current authority lived in the reference document's maintenance regime and commitments sections, rather than in that historical record.

The correction also changed what a review was for. Two recovery matrices—one for device loss, one for losing the machine and its nearby backup together—made the remaining exposure visible. They were a better guide to recovery work than the number of findings closed. P1-5, the second-copy problem, remained the dominant open recovery commitment. Reclassifying the backlog did not improve a single row's actual recoverability.
...
After the conversion, the same question has a recorded answer. For P2-17, fixing the pruning behavior is chosen work. For P2-11, no intent is asserted in either direction. The agent must understand those statements and check whether later evidence changes them. It need not infer the dispositions from severity labels, file modes, and conversational history.
...
I use **inference burden** here for the important operational state a reader must reconstruct indirectly because it is not explicit in authoritative artifacts. The phrase already exists; this is a local operational use, not a proposed coinage or a measured quantity.
...
Hutchins's distributed cognition is the broadest useful home for this observation. His study of naval navigation treats the team and its tools as a cognitive system, rather than attributing everything accomplished to individual navigators. Here the relevant unit includes the human operator, the agent, the repositories, the machine, and the procedures connecting them. The person who makes and authorizes decisions belongs in that picture as much as the artifact that preserves them. This is an application of the perspective, not a new result about minds. [Hutchins, 1995](https://pages.ucsd.edu/~ehutchins/citw.html); [Hutchins, 2014](https://pages.ucsd.edu/~ehutchins/documents/publishedCulturalEcosystemHumanCog.pdf)

Kirsh is a closer antecedent for the specific mechanism. In *Thinking with External Representations*, he asks why people draw, mark, rearrange, and annotate even when the environment supplies no information they could not in principle infer already. A different representation changes the work needed to reach a conclusion. Making a relationship explicit can make it easier to use. The underlying idea is already there; an operational repository gives it a particular application. [Kirsh, 2010](https://link.springer.com/article/10.1007/s00146-010-0272-8)

AI memory research supplies a close precedent too. PREMem performs synthesis and establishes relationships during memory construction so later responses need less reconstruction across conversations. Its authors explicitly distinguish successful retrieval from the reasoning that remains afterward. That narrows what this essay can claim as new. The mechanism is not missing from AI research. My case concerns which operational conclusions are authorized, how they remain current, and when leaving a question unresolved is the correct result. [Kim et al., 2025](https://aclanthology.org/2025.findings-emnlp.1204/)

None of this requires arguing that a repository is literally part of a model's mind. Clark and Chalmers make a stronger, constitutive argument about external resources under particular coupling conditions. This operational case does not establish the conditions needed to make that argument, and it does not need to. We can examine who performs the work without settling the boundaries of mind. [Clark and Chalmers, 1998](https://consc.net/papers/extended.html)
...
P2-11 makes the point especially clearly. Explicit uncertainty is useful state. A sufficiently capable model might propose explanations for the script's mode, but capability does not confer authority to select one as the operator's intent. Guessing can be impressive. It is still guessing.

P1-5 presents a different remaining task. Recording the dominant recovery exposure does not decide which offsite backup design best fits cost, privacy, bandwidth, and recovery needs. Those tradeoffs were not settled by the conversion. The next conversation can begin from the recorded exposure and work on the open choice.

Even the authority of a record is bounded. A historical conversion explains why an ID disappeared; it does not override a later commitment. An application-owned finding remains that application's to resolve. Permission to read either repository does not grant permission to change the live machine. These boundaries determine what an agent can conclude or do from the information it finds.
...
Agent-evaluation research gives another reason to keep those contributions distinct: a score reflects the surrounding execution and evaluation system as well as the model. That supports paying attention to task structure; it does not validate this workstation case. You want the joint system to work. You should still know which part you improved. [Zhang et al., 2026 preprint](https://arxiv.org/html/2609.09218v1)
...
Writing knowledge down was the beginning. The harder lesson was deciding what the record should require of its next reader. P2-17 should not need another conversation about whether anyone intends to fix it. P2-11 should not acquire an invented intention merely because the next agent dislikes an unanswered question.
...
| Anastasis conversion evidence | Frozen record dated September 17, 2026; preceding backlog also inspected | `docs/migrations/2026-09-17-commitment-model-conversion/README.md` | 4/6/2 disposition, P2-11 and P2-17 reasoning, transfers, and documentation-only scope; historical evidence, not current authority |
| Themis governance | Governing standards at `ea680ae`; relevant sections inspected | `docs/governance/PROJECT_REPORTING.md` §2; `PROJECT_WORK_CONTROL.md` §§2, 4, 7–10; `PROJECT_REGISTER_SPEC.md` §27 | System/instance separation, substantive evidence ownership, adoption, and native work-state semantics |
| Themis findings and investment methods | Operating methodology at `ea680ae`; inspected alongside governing standards | `docs/methodology/findings-queue-method.md`; `side-project-portfolio-framework.md` | Finding/obligation distinction and separate priority concepts; methodology is not a substitute for governing authority |
...
| P2-17's age-only pruning had emptied a local dump shelf | Reference §15, commitment text, conversion rationale | VERIFIED | Borg archives remained; this was loss of local dump copies, not demonstrated total database loss |
| P2-11 retained observed mode/history without asserting intent in either direction | Conversion table and reference §18 | VERIFIED | Does not establish that the mode was deliberate or defective |
...
| Current open commitments were P1-5, P2-9, P2-10, P2-12, P2-17 | Reference commitments at `2c73f67` | VERIFIED | Dated snapshot; four promoted P2 items plus the existing P1 item |
...
| Recovery matrices expose device-loss and site-loss consequences; P1-5 remains dominant for recovery | Reference §15 and commitments | VERIFIED | Bounded threat models, not a comprehensive quantitative measure of all workstation risk |
| Themis separates governing system, instance state, and externally owned substantive evidence | Reporting §2; Work Control §2; Register §27 | VERIFIED | System authority inspected; no portfolio instance or adoption manifest inspected |
| Themis has native work-state semantics and distinct priority concepts | Work Control §§4, 7–10 and investment methodology | VERIFIED | Adoption is conditional; does not import Anastasis terminology or imply one universal priority order |
...
| Agent scores depend on model, tools, scaffold, environment, and scorer | Zhang et al. September 2026 preprint, introduction and audit discussion | VERIFIED | A supported characterization of evaluation; the paper's interventions and scorer defects are benchmark-specific |
